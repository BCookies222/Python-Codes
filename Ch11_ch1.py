"""Challenge 1: Increase the difficulty as the game progresses.
+Increase the speed of the pizzas and chef. 
+Raise the pan to a higher level. 
+Increase the number of crazy chef's flinging pizzas."""
...
class Pan(games.Sprite): # Pan inherits from its super class (Sprite)
	...
	def __init__(self): 
		super(Pan,self).__init__(image=Pan.pan_pic, 
								x=games.mouse.x,
								bottom=games.screen.height-5) # Raised the pan from height to height-5
								
		self.score=games.Text(value=0, color=color.red, size=25, right=games.screen.width-10, top=5) 
...
class Pizza(games.Sprite):
	...
	speed=2 # Falling speed of Pizzas-increased from 1 to 2
	
	#set the initial values for the pizza-Constructor
	def __init__(self,x,y=90): # Place the pizza at chef's chest level ?? Not clear its not defined in angles
		super(Pizza,self).__init__(image=Pizza.pizza_pic, x=x,y=y,dy=Pizza.speed) #dx=? and what is x=x and y=y
	# Method to check the screen boundary for pizzas
	def update(self):
		if self.bottom > games.screen.height: # if the pizza moves out of the screen then call end_game() and destroy the pizza sprite object; 
			self.end_game()
			self.destroy()
...
class Chef(games.Sprite):
	...
	#Contsructor to initialize the chef ?? Why is x not defined for chef
	def __init__(self,y=55, speed=6,odds_change=200): # Increased the speed of chef dropping pizzas fall from 2 to 6 
	...
	def check_drop(self):
		...
		if self.time_til_drop >0:
			self.time_til_drop-=2 # Increased from 1 to 2 to show the change in number of pizzas dropping
		else:
			new_pizza=Pizza(x=self.x) # new pizza created at x position of the Chef Sprite
			games.screen.add(new_pizza)
			self.time_til_drop=int(new_pizza.height*1.1/Pizza.speed)+1 #Next pizza is dropped when the distance from the previous pizza  is about 
			#30% of pizza height(increasing the time delay b/w new pizza by 30%), independent of how fast the pizzas are falling -changed from 1.3 to 1.1 that should increase the #number of pizzas
def main():
