...
class Asteroid(Wrapper):
	...
	images = {SMALL  : games.load_image("asteroid_small.bmp"),
			  ...
			  EXTRA  : games.load_image("debris.jpg")} # Extra debris included
	SPEED = 2
	SPAWN = 2
	POINTS = 30
	total =  0
	...
	def die(self):
		...
		if self.size==Asteroid.SMALL:
			Asteroid.no_strikes=1
		# if asteroid isn't small, replace with two smaller asteroids
		if self.size==Asteroid.MEDIUM or self.size==Asteroid.LARGE:
			Asteroid.no_strikes=1
		...
		if self.size==Asteroid.EXTRA:
			Asteroid.no_strikes=2
...
class Ship(Collider):
	...
	def update(self):
		...
		if games.keyboard.is_pressed(games.K_SPACE ) * Asteroid.no_strikes and self.missile_wait == 0:
...
class Game(object):
	...
	def advance(self):
		...
		for i in range(self.level):
			...
			games.screen.add(new_asteroid)
			new_asteroid = Asteroid(game = self,x = x, y = y,size = Asteroid.EXTRA)
			games.screen.add(new_asteroid)
